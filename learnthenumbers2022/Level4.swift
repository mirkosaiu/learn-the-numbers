...
class Level4: SKSceneSuper {
    ...
    func showActualShadowWithChecktraks() {
        let showNumber = SKAction.move(to: CGPoint(x: actualShadowObject.position.x - marginOut, y: actualShadowObject.position.y), duration: 3)
        for ct in actualShadowChecktracks{
            let showCt = SKAction.move(to: CGPoint(x: ct.position.x - marginOut, y: ct.position.y), duration: 3)
            
            ct.run(showCt)
        }
        actualShadowObject.run(showNumber)
    ...
    override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        for touch in touches {
            ...
            trackHandler(touchPosition: pos)
        }
    ...
    func trackHandler (touchPosition: CGPoint) {
        ...
        for ct in actualShadowChecktracks {
            if actualShadowObject.contains(touchPosition) {
                if ct.contains(touchPosition) {
                    // punto iniziale
